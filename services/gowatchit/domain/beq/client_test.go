...
func TestCheckEdition(t *testing.T) {
	...
	type test struct {
		beqEdition string
		edition    mediaplayer.Edition
		expected   bool
	}
	tests := []test{
		{
			beqEdition: "Extended",
			edition:    mediaplayer.EditionExtended,
			expected:   true,
		},
		{
			beqEdition: "ex",
		...
		{
			beqEdition: "DC",
			edition:    mediaplayer.EditionDirectorsCut,
			expected:   true,
		},
		{
			beqEdition: "DC+SE+TC",
	...
	for i := range tests {
		...
		t.Run(fmt.Sprintf("Edition_%s", test.beqEdition), func(t *testing.T) {
			t.Parallel()
			match := checkEdition(&BeqCatalog{Edition: test.beqEdition}, test.edition)
