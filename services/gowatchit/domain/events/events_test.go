...
func TestMain(m *testing.M) {
	...
	dbOnce.Do(func() {
		...
		if err != nil {
			log.Fatalf("Failed to save ezbeq config: %v", err)
		}
...
func TestIsValidWebhook(t *testing.T) {
	...
	t.Run("Valid webhook", func(t *testing.T) {
		webhook := jellyfin.JellyfinWebhook{
			DeviceID:   "123",
			DeviceName: "TestDevice",
			ItemType:   "Movie",
		}
		assert.True(t, jellyfin.IsValidWebhook(&webhook))
	})

	t.Run("Invalid webhook - empty struct", func(t *testing.T) {
		webhook := jellyfin.JellyfinWebhook{}
		assert.False(t, jellyfin.IsValidWebhook(&webhook))
	})
