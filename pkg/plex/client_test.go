...
func TestMain(m *testing.M) {
	...
	dbOnce.Do(func() {
		...
		c, err = NewClient(ctx)
		if err != nil {
			l.Fatalf("Failed to create plex client: %v for values - %s, %s, %s", err, config.GetPlayerScheme(ctx), config.GetPlayerURL(ctx), config.GetPlayerPort(ctx))
		}
	...
	tests := []test{
		{
			...
			expected: editions.EditionUnrated,
		},
		{
			dataObj: MediaContainer{
				Video: struct {
		...
		},
		{
			...
			expected: editions.EditionExtended,
		},
		{
			dataObj: MediaContainer{
				Video: struct {
			...
			expected: editions.EditionDirectorsCut,
		},
		{
			dataObj: MediaContainer{
				Video: struct {
			...
			expected: editions.EditionSpecialEdition,
		},
		{
			dataObj: MediaContainer{
				Video: struct {
...
func TestMapCodecs(t *testing.T) {
	...
	for _, test := range tests {
		...
		a.Equal(codecs.Codec(test.expected), s)
	}
}
