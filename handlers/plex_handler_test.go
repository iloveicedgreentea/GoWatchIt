...
func generateMultipartPayload(filePath string) (*bytes.Buffer, string, error) {
	...
	payloadPart.Write(jsonData)

	// Add file part
	filePart, err := writer.CreateFormFile("file", "example.txt")
	if err != nil {
		return nil, "", err
	}
	filePart.Write([]byte("This is the content of the file."))
...
func TestEndToEnd(t *testing.T) {
	assert.NoError(t, sendMulitpart("testdata/media.play.priv.json", t))
	time.Sleep(7 * time.Second)
	assert.NoError(t, sendMulitpart("testdata/media.pause.priv.json", t))
}

func sendMulitpart(filePath string, t *testing.T) error {
	multipartBody, contentType, err := generateMultipartPayload(filePath)
	if err != nil {
		return fmt.Errorf("Error generating multipart payload: %v", err)
	}

	url := "http://localhost:9999/plexwebhook"
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	// Create a new HTTP POST request
	request, err := http.NewRequestWithContext(ctx, "POST", url, multipartBody)
	if err != nil {
	...
	if response.StatusCode != 200 {
		return fmt.Errorf("Response code is not 200: %d", response.StatusCode)
	}
