...
func TestProcessWebhook(t *testing.T) {
    ...
    // If you want to inspect the JSON response returned by the function, you can do that too
    var jsonResponse map[string]interface{}
    err := json.Unmarshal(respRecorder.Body.Bytes(), &jsonResponse)
    assert.NoError(t, err)

    // Assuming the function returns a message like {"message": "Payload processed"}
